...
const affirmations = [
    ...
    { text: "It's okay to rest. You don't have to earn your right to take a break.", icon: CloudSun, theme: 'sky' }, // teal family
    ...
    { text: "Asking for help is a sign of strength, not weakness.", icon: CloudSun, theme: 'sky' }, // teal family
...
const themeMap: Record<string, { bg: string; border: string; text: string; glow: string }> = {
    ...
    sky: { bg: 'from-teal-50 to-cyan-50', border: 'border-teal-200/60', text: 'text-teal-700', glow: 'bg-teal-200/30' },
                            ...
                            <div className="flex gap-1">
                                {affirmations.map((_, i) => (
                                    <button
                                        key={i}
                                        onClick={() => setIndex(i)}
                                        className={`w-1.5 h-1.5 rounded-full transition-all ${i === index ? `${theme.text} bg-current scale-125` : 'bg-gray-300/50 dark:bg-border'}`}
                                    />
                                ))}
                            </div>
                        </div>
