...
/**
 * Radix packages reachable from the eagerly-mounted app shell: <Toaster>
 * (react-toast) and <TooltipProvider> (react-tooltip), plus everything those
 * two pull in transitively. Anything not listed here is only reachable from a
 * lazy route and is chunked separately. See the manualChunks note below.
 */
const RADIX_EAGER = new Set([
  // the two components the shell actually mounts
  "react-toast",
  "react-tooltip",
    ...
    rollupOptions: {
      output: {
        ...
        manualChunks(rawId) {
          ...
          // ── Radix, split by who needs it on first paint ───────────────
          // Grouping the whole family into one chunk put ~80 KB of it on the
          // critical path to serve two components. The app shell eagerly
          // mounts <Toaster> (react-toast) and <TooltipProvider>
          // (react-tooltip); select, menu, slider, radio-group, dialog and the
          // rest belong to lazy routes and rode along, because Rollup emits a
          // modulepreload for every chunk the entry can reach statically.
          //
          // An allow-list of package names rather than a pattern: the eager
          // pair's transitive deps include packages with no `react-` prefix
          // (@radix-ui/primitive, rect, number). A regex that assumed the
          // prefix left those in the lazy chunk, core imported them from
          // there, and the edge dragged the whole lazy chunk back onto the
          // critical path — the split looked applied and bought nothing.
          //
          // Adding an eagerly-mounted Radix component means adding it AND its
          // deps here. Verify with:
          //   grep -o 'vendor-radix-[a-z]*' dist/index.html | sort -u
          // which must print vendor-radix-core and NOT vendor-radix-lazy.
          if (id.includes("@radix-ui")) {
            const pkg = id.match(/@radix-ui\/([^/]+)/)?.[1] ?? "";
            return RADIX_EAGER.has(pkg) ? "vendor-radix-core" : "vendor-radix-lazy";
          }
