...
export default function ToolShell({
  ...
  return (
    <div className={`mindgym-root min-h-screen relative overflow-hidden flex flex-col items-center select-none bg-gradient-to-b ${themeColor} transition-colors duration-[2s]`}>
      ...
      <div className="fixed top-2 left-0 w-full z-40 px-4 pt-4 sm:pt-6 flex justify-center pointer-events-none">
        <div className="w-full max-w-2xl pointer-events-auto">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl flex items-center justify-between px-3 py-2.5 shadow-2xl">
            <button
              onClick={() => navigate("/mindgym")}
              className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-all"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
            
            <div className="flex flex-col items-center justify-center mx-2 flex-grow overflow-hidden">
              <h1 className="text-sm font-medium text-white/90 drop-shadow-sm truncate w-full text-center tracking-wide">{title}</h1>
              {totalSteps != null && currentStep != null && (
                <div className="w-24 h-1 bg-black/20 rounded-full mt-1.5 overflow-hidden flex shadow-inner">
                  {Array.from({ length: totalSteps }, (_, i) => (
                    <div
                      key={i}
                      className={`h-full flex-1 transition-all duration-500 ease-out border-r border-black/10 last:border-0 ${
                        i < currentStep ? theme.bg : i === currentStep ? `${theme.bg} opacity-50` : "bg-transparent"
                      }`}
                    />
                  ))}
                </div>
            ...
            <button
              ...
              className={`flex items-center justify-center h-10 px-3 rounded-full hover:bg-white/5 transition-all outline-none ${whyOpen ? 'bg-white/10 text-white' : 'text-white/50 hover:text-white/90'}`}
          ...
          <AnimatePresence>
            {whyOpen && (
              <motion.div
                ...
                className="mx-auto w-[90%] bg-black/40 backdrop-blur-2xl border border-white/10 rounded-2xl p-4 shadow-2xl"
              >
                <div className="flex items-start gap-3">
                  <div className={`p-2 rounded-xl bg-white/5 ${theme.accent}`}>
                    <Sparkles className="w-4 h-4" />
                  </div>
                  <div>
                    <h3 className="text-xs font-semibold text-white/90 uppercase tracking-wider mb-1">Clinical Basis</h3>
      ...
      <AnimatePresence mode="wait">
        {completed ? (
          ...
          <motion.div
            key="content"
            className="flex-1 w-full flex flex-col justify-center items-center z-10 relative pt-24 pb-8 min-h-[calc(100vh-100px)]"
      ...
      <div className="fixed bottom-safe right-4 z-40 pb-4">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-rose-500/10 backdrop-blur-md border border-rose-500/20 text-rose-300 text-xs font-semibold hover:bg-rose-500/20 shadow-lg transition-all"
        >
