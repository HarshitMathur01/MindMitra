...
class LipsyncEn {
  ...
  */
  constructor() {
    ...
    // Stretched ~15-20% for more natural, unhurried mouth movements
    this.visemeDurations = {
      'aa': 1.12, 'E': 1.06, 'I': 1.08, 'O': 1.14, 'U': 1.12, 'PP': 1.25,
      'SS': 1.42, 'TH': 1.18, 'DD': 1.22, 'FF': 1.18, 'kk': 1.40, 'nn': 1.04,
      'RR': 1.04, 'CH': 1.28, 'sil': 1.20
    };
    ...
    // Groups: vowels blend more, stops create discontinuities
    this.visemeGroup = {
      'aa': 'vowel', 'E': 'vowel', 'I': 'vowel', 'O': 'vowel', 'U': 'vowel',
      'PP': 'stop', 'DD': 'stop', 'kk': 'stop',
      'SS': 'fricative', 'TH': 'fricative', 'FF': 'fricative', 'CH': 'fricative',
      'nn': 'nasal', 'RR': 'liquid', 'sil': 'silence'
    };
    ...
    // Increased for more natural breathing room between words/phrases
  ...
  * Returns enhanced data including per-viseme intensity, jaw coupling,
  * and coarticulation weights for smoother, more natural lip movement.
  * @param {string} w Text
  * @return {Object} Oculus LipSync Visemes, durations, intensities, jaw & coarticulation.
  */
  wordsToVisemes(w) {
    let o = {
    ...
    // Compute coarticulation weights (anticipatory blending with next viseme)
    for (let i = 0; i < o.visemes.length; i++) {
      const currGroup = this.visemeGroup[o.visemes[i]] || 'silence';
      const nextGroup = (i < o.visemes.length - 1)
        ? (this.visemeGroup[o.visemes[i + 1]] || 'silence')
        : 'silence';
      o.coartWeights.push(
        (this.coarticulationForward[currGroup] && this.coarticulationForward[currGroup][nextGroup]) || 0
      );
    }
