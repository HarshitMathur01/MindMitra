...
function buildBhangraDance(arm) {
  ...
  // Kaintha arms — held out in FRONT of the body (forward reach) with a
  // beat-synced vertical pump.
  ...
  // Axes: Z abducts (lifts the arm; right +, left −) and Y swings it forward to
  // the front (right −, left +, same sign as the Double clap / Thinking reach).
  // The big Y holds the arm pointing forward; Z pumps its height per beat.
  const lUp = [0, -105 * D2R, -75 * D2R];
  const lDown = [0, -105 * D2R, -45 * D2R];
  const rUp = [0, 105 * D2R, 75 * D2R];
  const rDown = [0, 105 * D2R, 45 * D2R];
  pushQuat(tracks, used, lUA, t, [
    [0, 0, 0],
    [0, -85 * D2R, -40 * D2R], lDown,
    lUp, lDown,
    lUp, lDown,
    lUp, lDown,
    lUp, lDown,
  ...
  // Forearms bend forward so the hands lead out ahead of the body. Held steady
  // across the clip (the pump comes from the upper arm). No-op on rigs that
  // lack forearm bones.
  const lLA = arm.resolved.leftLowerArm;
  const rLA = arm.resolved.rightLowerArm;
  const fwdBend = (deg) => t.map((_, i) =>
    (i === 0 || i === t.length - 1) ? [0, 0, 0] : [0, deg * D2R, 0]);
  pushQuat(tracks, used, lLA, t, fwdBend(-32));
  pushQuat(tracks, used, rLA, t, fwdBend(32));
  ...
  // Open palms — fingers extended and splayed, held through the clip
  // (a flat raised-hand celebration rather than the pointing kaintha). Both
  // hands share the same shape; spread is mirrored per side inside pushPalmShape.
  const openCurl = degKeys([
    0, 2, 3, 3, 3, 3, 3, 3, 3,
    3, 3, 3, 3, 3, 3, 2, 0,
  ]);
  const openSpread = degKeys([
    0, 8, 11, 11, 11, 11, 11, 11, 11,
    11, 11, 11, 11, 11, 11, 8, 0,
  ]);
  pushPalmShape(tracks, used, arm, 'left', t, openCurl, openSpread);
  pushPalmShape(tracks, used, arm, 'right', t, openCurl, openSpread);
